...
bool Toolbar::init()
{
	...
	ui::Button* pBtn = nullptr;
	{
		...
		pBtn = dynamic_cast<ui::Button*>(tool_widget->getChildByName("shop"));
		pBtn->addTouchEventListener( std::bind(&Toolbar::onClickShop, this, placeholders::_1, placeholders::_2) );
	}
...
void Toolbar::onClickHoe( Ref* target, cocos2d::ui::Widget::TouchEventType type )
{
	...
	if (nullptr!=mainlayer->getTopLayer() || !gamemap->onEffectHoe())
	{
...
void Toolbar::onClickBomb( Ref* target, cocos2d::ui::Widget::TouchEventType type )
{
	...
	if (nullptr!=mainlayer->getTopLayer() || !gamemap->onEffectBomb())
	{
		playEffect(ClickError);
	}
	else
...
void Toolbar::onClickMap( Ref* target, cocos2d::ui::Widget::TouchEventType type )
{
	...
	if (nullptr!=mainlayer->getTopLayer() || !gamemap->onEffectMap())
	{
		playEffect(ClickError);
	}
	else
...
void Toolbar::onClickShop( Ref* target, cocos2d::ui::Widget::TouchEventType type )
{
	Return_If(type != cocos2d::ui::Widget::TouchEventType::ENDED);
	Layer* layer = mainlayer->getTopLayer();
	if (layer != nullptr)
	{
		playEffect(ClickError);
		return;
	}
	mainlayer->popLayer( ShopLayer::create() );
	playEffect(ClickButton);
}
